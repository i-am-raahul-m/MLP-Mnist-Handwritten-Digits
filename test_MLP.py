...
def input_image_tensor(test_idx):
    image_tensor = torch.tensor(images[test_idx], dtype=torch.float32)
    image_tensor /= 255
    return image_tensor.unsqueeze(0)

# Dataset Label input tensor creation
def input_label(test_idx):
    label_tensor = torch.tensor(labels[test_idx])
...
# Loading saved model
mlp = MLP(input_dim=data_dim, output_dim=num_classes)
mlp.load_state_dict(torch.load(model_file_path, weights_only=True))
mlp.eval()
